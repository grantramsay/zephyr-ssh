...
struct ssh_channel *ssh_connection_allocate_channel(struct ssh_transport *transport)
{
	...
	if (channel != NULL) {
		...
		ring_buf_init(&channel->rx_ring_buf, sizeof(channel->rx_ring_buf_data), channel->rx_ring_buf_data);
		ring_buf_init(&channel->rx_stderr_ring_buf, sizeof(channel->rx_stderr_ring_buf_data), channel->rx_stderr_ring_buf_data);
		channel->open = false;
		channel->in_use = true;
	}
...
struct ssh_channel *ssh_connection_get_channel(struct ssh_transport *transport, uint32_t local_channel)
{
	struct ssh_channel *channel = NULL;
			    ...
			    ssh_channel_event_callback_t callback, void *user_data)
{
	...
	struct ssh_transport *transport = channel->transport;
...
int ssh_connection_process_msg(struct ssh_transport *transport, uint8_t msg_id, struct ssh_payload *rx_pkt)
{
	int res = -1;

	switch (msg_id) {
	case SSH_MSG_GLOBAL_REQUEST: {
		LOG_INF("GLOBAL_REQUEST");
		...
		LOG_INF("GLOBAL_REQUEST: %.*s want reply: %s", request_name.len, request_name.data,
			want_reply ? "true" : "false");
		res = 0;
		break;
	}
	case SSH_MSG_REQUEST_SUCCESS: {
		LOG_INF("REQUEST_SUCCESS");
		res = 0;
		break;
	}
	case SSH_MSG_REQUEST_FAILURE: {
		LOG_INF("REQUEST_FAILURE");
		res = 0;
		break;
	}
#ifdef CONFIG_SSH_SERVER
	case SSH_MSG_CHANNEL_OPEN: {
		LOG_INF("CHANNEL_OPEN");
		// Server only
		if (!transport->server) {
			break;
		}
		...
		static const struct ssh_string supported_channel_type = SSH_STRING_LITERAL("session");
		if (!ssh_strings_equal(&channel_type, &supported_channel_type)) {
			LOG_WRN("Unsupported channel type");
			return -1;
		}
	...
	case SSH_MSG_CHANNEL_REQUEST: {
		...
		LOG_INF("CHANNEL_REQUEST: %u %.*s want reply: %s",
			recipient_channel, request_type.len, request_type.data,
			want_reply ? "true" : "false");

		struct ssh_channel *channel = ssh_connection_get_channel(transport, recipient_channel);
		if (channel == NULL) {
		...
		static const struct {
			...
			{ .type = SSH_CHANNEL_REQUEST_PSEUDO_TERMINAL,	.string = SSH_STRING_LITERAL("pty-req") },
			{ .type = SSH_CHANNEL_REQUEST_X11_FORWARD,	.string = SSH_STRING_LITERAL("x11-req") },
			{ .type = SSH_CHANNEL_REQUEST_ENV_VAR,		.string = SSH_STRING_LITERAL("env") },
			{ .type = SSH_CHANNEL_REQUEST_SHELL,		.string = SSH_STRING_LITERAL("shell") },
			{ .type = SSH_CHANNEL_REQUEST_EXEC,		.string = SSH_STRING_LITERAL("exec") },
			{ .type = SSH_CHANNEL_REQUEST_SUBSYSTEM,	.string = SSH_STRING_LITERAL("subsystem") },
			{ .type = SSH_CHANNEL_REQUEST_WINDOW_CHANGE,	.string = SSH_STRING_LITERAL("window-change") },
			{ .type = SSH_CHANNEL_REQUEST_FLOW_CONTROL,	.string = SSH_STRING_LITERAL("xon-xoff") },
			{ .type = SSH_CHANNEL_REQUEST_SIGNAL,		.string = SSH_STRING_LITERAL("signal") },
			{ .type = SSH_CHANNEL_REQUEST_EXIT_STATUS,	.string = SSH_STRING_LITERAL("exit-status") },
			{ .type = SSH_CHANNEL_REQUEST_EXIT_SIGNAL,	.string = SSH_STRING_LITERAL("exit-signal") }
		};

		enum ssh_channel_request_type req_type = SSH_CHANNEL_REQUEST_UNKNOWN;
		...
		if (req_type == SSH_CHANNEL_REQUEST_PSEUDO_TERMINAL) {
			...
			LOG_INF("PSEUDO_TERMINAL: %.*s %u %u %u %u", term_env.len, term_env.data,
				term_width_chars, term_height_rows, term_width_pixels, term_height_pixels);
			LOG_HEXDUMP_INF(term_encoded_modes.data, term_encoded_modes.len, "PSEUDO_TERMINAL modes:");
		}
	...
	case SSH_MSG_CHANNEL_OPEN_CONFIRMATION: {
		LOG_INF("CHANNEL_OPEN_CONFIRMATION");
		// Client only
		if (transport->server) {
			break;
		}
		uint32_t recipient_channel, sender_channel, initial_window_size, maximum_packet_size;
		if (!ssh_payload_read_u32(rx_pkt, &recipient_channel) ||
		    !ssh_payload_read_u32(rx_pkt, &sender_channel) ||
		    !ssh_payload_read_u32(rx_pkt, &initial_window_size) ||
		    !ssh_payload_read_u32(rx_pkt, &maximum_packet_size)) {
	...
	case SSH_MSG_CHANNEL_DATA: {
		...
		LOG_HEXDUMP_DBG(data.data, data.len, "Data:");
		...
		uint32_t written = ring_buf_put(&channel->rx_ring_buf, data.data, data.len);
		if (written < data.len) {
			LOG_ERR("Flow control error");
			return -1;
		}
	...
	case SSH_MSG_CHANNEL_EXTENDED_DATA: {
		...
		LOG_HEXDUMP_DBG(data.data, data.len, "Data ext:");
		...
		if (data_type_code == SSH_EXTENDED_DATA_STDERR) {
			uint32_t written = ring_buf_put(&channel->rx_stderr_ring_buf, data.data, data.len);
			if (written < data.len) {
				LOG_ERR("Flow control error");
				return -1;
			}
...
#ifdef CONFIG_SSH_CLIENT
int ssh_connection_send_channel_open(
	struct ssh_transport *transport, const struct ssh_string *channel_type,
	uint32_t sender_channel, uint32_t initial_window_size, uint32_t maximum_packet_size)
{
	int res;
	...
	if (res < 0) {
		LOG_WRN("Failed to send CHANNEL_OPEN %d", res);
	}
	return res;
}
#endif

int ssh_connection_send_channel_request_pseudo_terminal(
	struct ssh_transport *transport, uint32_t recipient_channel, bool want_reply)
{
	int res;
	SSH_PAYLOAD_BUF(payload, transport->tx_buf);
	...
	static const uint8_t term_modes_blob[] = {
		,
	...
	uint32_t term_width_chars = 80, term_height_rows = 24,
	...
	res = ssh_transport_send_packet(transport, &payload);
	if (res < 0) {
		LOG_WRN("Failed to send CHANNEL_REQUEST %d", res);
	}
	return res;
}

int ssh_connection_send_channel_request(struct ssh_transport *transport, uint32_t recipient_channel,
					const struct ssh_string *channel_type, bool want_reply)
{
	int res;
	SSH_PAYLOAD_BUF(payload, transport->tx_buf);
...
int ssh_connection_send_channel_data(
	struct ssh_transport *transport, uint32_t recipient_channel,
	const void *data, uint32_t data_len)
{
	...
	res = ssh_transport_send_packet(transport, &payload);
	if (res < 0) {
		LOG_WRN("Failed to send CHANNEL_DATA %d", res);
	}
	return res;
}

int ssh_connection_send_channel_extended_data(
	struct ssh_transport *transport, uint32_t recipient_channel,
	uint32_t data_type_code, const void *data, uint32_t data_len)
{
	int res;
	SSH_PAYLOAD_BUF(payload, transport->tx_buf);
	struct ssh_string data_str = {
	...
	res = ssh_transport_send_packet(transport, &payload);
	if (res < 0) {
		LOG_WRN("Failed to send CHANNEL_EXTENDED_DATA %d", res);
	}
	return res;
}

int ssh_connection_send_window_adjust(
	struct ssh_transport *transport, uint32_t recipient_channel, uint32_t bytes_to_add)
{
	int res;
	SSH_PAYLOAD_BUF(payload, transport->tx_buf);
	...
	res = ssh_transport_send_packet(transport, &payload);
	if (res < 0) {
		LOG_WRN("Failed to send CHANNEL_WINDOW_ADJUST %d", res);
	}
	return res;
}

#ifdef CONFIG_SSH_SERVER
int ssh_connection_send_channel_open_confirmation(
	struct ssh_transport *transport, uint32_t recipient_channel,
	uint32_t sender_channel, uint32_t initial_window_size, uint32_t maximum_packet_size)
{
	int res;
	...
	res = ssh_transport_send_packet(transport, &payload);
	if (res < 0) {
		LOG_WRN("Failed to send CHANNEL_OPEN_CONFIRMATION %d", res);
	}
	return res;
}
#endif

int ssh_connection_send_channel_result(struct ssh_transport *transport, bool success, uint32_t recipient_channel)
{
	int res;
	SSH_PAYLOAD_BUF(payload, transport->tx_buf);
	...
	res = ssh_transport_send_packet(transport, &payload);
	if (res < 0) {
		LOG_WRN("Failed to send CHANNEL_SUCCESS %d", res);
	}
	return res;
}
