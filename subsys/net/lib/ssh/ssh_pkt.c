...
// Checks that adding the given length to the payload will not exceed the payload size or overflow
...
{
	struct ssh_string ssh_str = { .data = str, .len = strlen(str) };
...
bool ssh_payload_write_mpint(struct ssh_payload *payload, const uint8_t *data, uint32_t len, bool is_signed)
{
...
bool ssh_payload_write_name_list(struct ssh_payload *payload, const struct ssh_string *names, uint32_t n)
{
...
bool ssh_payload_read_name_list(struct ssh_payload *payload, struct ssh_payload *name_list)
{
	...
	bool res = ssh_payload_read_u32(payload, &string.len) &&
		   ssh_payload_read_raw(payload, &string.data, string.len);
	if (res && name_list != NULL) {
...
bool ssh_payload_name_list_iter(struct ssh_payload *name_list, struct ssh_string *name_out)
{
	...
	size_t remaining = name_list->size - name_list->len;
	name_out->data = &name_list->data[name_list->len];
	const uint8_t *comma = memchr(name_out->data, ',', remaining);
	if (comma == NULL) {
