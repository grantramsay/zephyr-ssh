...
struct ssh_transport {
	...
	uint32_t kexinit_sent : 1;
	uint32_t kexinit_received : 1;
	uint32_t newkeys_sent : 1;
	...
	mbedtls_ecdsa_context ecdsa_ephemeral_key;

	struct ssh_cipher_aes128_ctr {
		mbedtls_aes_context aes_crypt;
		uint8_t aes_stream_block[MBEDTLS_AES_BLOCK_SIZE];
		size_t aes_nc_off;
	} tx_cipher, rx_cipher;
...
struct ssh_server {
	...
	K_KERNEL_STACK_MEMBER(thread_stack, 4096);
};
#endif
