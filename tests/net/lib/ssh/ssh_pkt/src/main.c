...
ZTEST(ssh_pkt, test_write_mpint)
{
	...
	static const uint8_t tc_1_expected[] = { 0x00, 0x00, 0x00, 0x08, 0x09, 0xa3, 0x78, 0xf9, 0xb2, 0xe3, 0x32, 0xa7 };
	...
	static const uint8_t tc_4_expected[] = { 0x00, 0x00, 0x00, 0x05, 0xff, 0x21, 0x52, 0x41, 0x11};
	...
	static const struct test_case {
		...
		{.data = tc_0_data, .data_len = sizeof(tc_0_data), .expected = tc_0_expected, .expected_len = sizeof(tc_0_expected), false},
		{.data = tc_1_data, .data_len = sizeof(tc_1_data), .expected = tc_1_expected, .expected_len = sizeof(tc_1_expected), false},
		{.data = tc_2_data, .data_len = sizeof(tc_2_data), .expected = tc_2_expected, .expected_len = sizeof(tc_2_expected), false},
		{.data = tc_3_data, .data_len = sizeof(tc_3_data), .expected = tc_3_expected, .expected_len = sizeof(tc_3_expected), true},
		{.data = tc_4_data, .data_len = sizeof(tc_4_data), .expected = tc_4_expected, .expected_len = sizeof(tc_4_expected), true},
		{.data = tc_5_data, .data_len = sizeof(tc_5_data), .expected = tc_5_expected, .expected_len = sizeof(tc_5_expected), false},
		{.data = tc_6_data, .data_len = sizeof(tc_6_data), .expected = tc_6_expected, .expected_len = sizeof(tc_6_expected), true},
	};

	for (unsigned i = 0; i < ARRAY_SIZE(test_cases); i++) {
		...
		struct ssh_payload null_payload = {.size = UINT32_MAX};
		zassert_true(ssh_payload_write_mpint(
